...
def cross_entropy(input, target, size_average=True, ignore_value=-100):
    ...
    if size_average:
        ...
        return torch.sum(torch.sum(-(target * ignore_mask) * logsoftmax(input), dim=1))
...
@add_start_docstrings("""Compact RoBERTa Model with a `language modeling` head on top. """, ROBERTA_START_DOCSTRING)
class CompactRobertaForMaskedLM(RobertaForMaskedLM):
    ...
    def forward(
        ...
        mask_positions = mask_positions.flatten()
        mask_positions = mask_positions.nonzero()
        prediction_scores = prediction_scores.view((-1, prediction_scores.size()[-1]))[mask_positions].squeeze()

        class_token_idxs = [self.tokenizer.encode(self.verbalizer.classes[label])[1] for label in self.args.labels]
        prediction_scores = prediction_scores[:, class_token_idxs]
